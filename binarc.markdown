...
* binarcは、 C++での**デシリアライズで、動的なメモリ確保を行いません** 。
...
- 書庫のワード#0には、0x526f6f54（C言語で記述すると'RooT'）が格納されています。
  - ワード#0をバイト列として取得した値によって、書庫のエンディアンが判定できます。
    - バイト列が\[0x54, 0x6f, 0x6f, 0x52\]なら、リトルエンディアン。
    - バイト列が\[0x52, 0x6f, 0x6f, 0x54\]なら、ビッグエンディアン。
- 書庫のワード#1には、木構造の最上位の値タグが格納されています。
- 書庫のワード#2以降には、木構造のすべての値がバイト列にシリアライズされて格納されています。
...
|ワードアドレス          |ワードの内容             |
|------------------------|-------------------------|
|ワード#0                |IEEE754倍精度浮動小数点数|
|ワード#1                |〃                       |

#### UTF-8文字列

- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体のワード#0に、UTF-8文字列のバイト数が格納されています。
  - 値本体のワード#1に、UTF-8文字列の値ハッシュが格納されています。
  - 値本体のワード#2以降に、UTF-8文字列が格納されています。
- 値本体は、(2 + (文字列のバイト数 + 3) / 4)ワードを使って格納されています。
- 値ハッシュは、[32ビットのFNV-1ハッシュ](http://www.radiumsoftware.com/0605.html#060526)です。

|ワードアドレス          |ワードの内容           |
|------------------------|-----------------------|
|ワード#0                |UTF-8文字列のバイト数n |
|ワード#1                |UTF-8文字列の値ハッシュ|
|ワード#2                |UTF-8文字列            |
|（省略）                |〃                     |
|ワード#(1 + (n + 3) / 4)|〃                     |
...
- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体のワード#0に、拡張型バイト列のバイト数が格納されています。
  - 値本体のワード#1に、拡張型バイト列の値ハッシュが格納されています。
  - 値本体のワード#2に、拡張型バイト列の種別が格納されています。
    - 値域[0, 0x7fffffff]は予約されているため、使用できません。
  - 値本体のワード#3以降に、拡張型バイト列が格納されています。
- 値本体は、(3 + (拡張型バイト列のバイト数 + 3) / 4)ワードを使って格納されています。
- 値ハッシュは、[32ビットのFNV-1ハッシュ](http://www.radiumsoftware.com/0605.html#060526)です。
...
- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体のワード#0に、配列の要素数が格納されています。
  - 値本体のワード#1以降に、配列要素の値タグが連続して格納されています。
- 値本体は、(1 + 配列の要素数)ワードを使って格納されています。
- 値ハッシュは、必ず0xffffffffとなります。
...
- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体のワード#0に、辞書の要素数が格納されています。
  - 値本体のワード#1以降に、要素マップ値の値タグが連続して格納されています。
    - 辞書の要素マップ値は、要素キーの値ハッシュの昇順でソートされています。
  - 値本体のワード#nに、辞書の要素数が格納されています。
  - 値本体のワード#(n + 1)以降に、要素キーの値タグが連続して格納されています。
    - 辞書の要素キーは、必ずスカラー型の値です。コンテナ型の値を辞書の要素キーに使うことはできません。
    - 辞書の要素キーは、要素キーの値ハッシュの昇順でソートされています。
      - 要素キーの値ハッシュが等価な場合は、要素キーの格納形式の昇順となります。
- 値本体は、(2 + 辞書の要素数 * 2)ワードを使って格納されています。
- 値ハッシュは、必ず0xffffffffとなります。
...
|ワード#3          |要素#2マップ値の値タグ      |
|（省略）          |（省略）                    |
|ワード#(n - 3)    |要素#(n - 3)マップ値の値タグ|
|ワード#(n - 2)    |要素#(n - 2)マップ値の値タグ|
|ワード#(n - 1)    |要素#(n - 1)マップ値の値タグ|
|ワード#n          |辞書の要素数n               |
|ワード#(n + 1)    |要素#0キーの値タグ          |
|ワード#(n + 2)    |要素#1キーの値タグ          |
|ワード#(n + 3)    |要素#2キーの値タグ          |
|（省略）          |（省略）                    |
|ワード#(n * 2 - 1)|要素#(n - 3)キーの値タグ    |
|ワード#(n * 2)    |要素#(n - 2)キーの値タグ    |
|ワード#(n * 2 + 1)|要素#(n - 1)マップ値の値タグ|
