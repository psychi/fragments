...
- 書庫のワード#0には、0x494e4478（C言語で記述すると'INDx'）が格納されています。
...
- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体のワード#0に、(辞書の要素数 * 2)が格納されています。
  - 値本体のワード#1以降に、要素キーの値タグと要素値の値タグのペアが、要素キーの値ハッシュの昇順で連続して格納されています。
    - 辞書の要素キーは、必ずスカラー型の値です。コンテナ型の値を辞書の要素キーに使うことはできません。
- 値本体は、(1 + 辞書の要素数 * 2)ワードを使って格納されています。
- 辞書の値ハッシュは、必ず0xffffffffとなります。
