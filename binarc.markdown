...
- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体の先頭ワードに、UTF-8文字列のバイト数が格納されています。
  - 値本体の2ワード目に、UTF-8文字列の値ハッシュが格納されています。
  - 値本体の3ワード目以降に、UTF-8文字列が格納されています。
- 値本体は、(2 + (文字列のバイト数 + 3) / 4)ワードを使って格納されています。
- UTF-8文字列の値ハッシュは、[32ビットのFNV-1ハッシュ](http://www.radiumsoftware.com/0605.html#060526)です。

#### 拡張型バイト列

- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体の先頭ワードに、拡張型バイト列のバイト数が格納されています。
  - 値本体の2ワード目に、拡張型バイト列の値ハッシュが格納されています。
  - 値本体の3ワード目に、拡張型の種別が格納されています。
    - 値域[0, 0x7fffffff]は、拡張型の種別として予約されているため、使用できません。
  - 値本体の4ワード目以降に、拡張型バイト列が格納されています。
- 値本体は、(3 + (拡張型バイト列のバイト数 + 3) / 4)ワードを使って格納されています。
- 拡張型バイト列の値ハッシュは、[32ビットのFNV-1ハッシュ](http://www.radiumsoftware.com/0605.html#060526)です。

#### 配列コンテナ

- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体の先頭ワードに、配列の要素数が格納されています。
  - 値本体の2ワード目以降に、配列要素の値タグが連続して格納されています。
- 値本体は、(1 + 配列の要素数)ワードを使って格納されています。
- 配列の値ハッシュは、必ず0xffffffffとなります。

#### 辞書コンテナ

- 値本体は、binarc書庫の先頭位置を基準位置、値タグのインデックス部をオフセットワード数とした場所に格納されています。
  - 値本体の先頭ワードに、(辞書の要素数 * 2)が格納されています。
  - 値本体の2ワード目以降に、要素キーの値タグと要素値の値タグのペアが、要素キーの値ハッシュの昇順で連続して格納されています。
    - 辞書の要素キーは、必ずスカラー型です。コンテナ型を辞書の要素キーに使うことはできません。
