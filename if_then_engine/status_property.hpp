...
/** @brief 状態値が格納されているビット列チャンクの場所を示す。
...
class psyq::if_then_engine::_private::status_property
{
    ...
    static_assert(
        sizeof(template_bit_position) <= sizeof(template_pack),
        "sizeof(template_bit_position) is not less than sizeof(template_pack).");
    ...
    static_assert(
        sizeof(template_bit_width) < sizeof(template_bit_position),
        "sizeof(template_bit_width) is not less than sizeof(template_bit_position).");
    ...
    public: enum pack_format: std::uint8_t
    {
        ...
        pack_POSITION_BACK =
            sizeof(template_bit_position) < sizeof(template_pack)?
                sizeof(template_bit_position) * psyq::CHAR_BIT_WIDTH - 1:
                sizeof(typename this_type::pack) * psyq::CHAR_BIT_WIDTH - 2
                    - sizeof(typename this_type::format) * psyq::CHAR_BIT_WIDTH,

        /// @brief 状態値のビット構成の先頭ビット位置。
        pack_FORMAT_FRONT,

        /// @brief 状態値のビット構成の末尾ビット位置。
        pack_FORMAT_BACK = pack_FORMAT_FRONT
            + sizeof(typename this_type::format) * psyq::CHAR_BIT_WIDTH - 1,

        /// @brief 状態変化フラグのビット位置。
        pack_TRANSITION_POSITION,
    };
    static_assert(
        // ビット位置の最大値が
        ...
        "");
    static_assert(
        this_type::pack_TRANSITION_POSITION
            < sizeof(typename this_type::pack) * psyq::CHAR_BIT_WIDTH,
        "Bit width of template_pack is not greater than pack_FORMAT_BACK.");

    public: enum pack_mask: typename this_type::pack
    {
        /// @brief 状態値が格納されているビット位置を取得するためのビットマスク。
        pack_POSITION_MASK = ~(
            ~typename this_type::pack(0)
            << (pack_POSITION_BACK - pack_POSITION_FRONT + 1)),

        /// @brief 状態値のビット構成を取得するためのビットマスク。
        pack_FORMAT_MASK = ~(
            ~typename this_type::pack(0)
            << (pack_FORMAT_BACK - pack_FORMAT_FRONT + 1)),
    };
    ...
    pack_(
        this_type::set_format(
            typename this_type::pack(1) << this_type::pack_TRANSITION_POSITION,
            in_format)),
    ...
    {
        return 1 & (this->pack_ >> this_type::pack_TRANSITION_POSITION);
    }
    ...
    {
        this->pack_ = psyq::set_bit_fast(
            this->pack_,
            this_type::pack_TRANSITION_POSITION,
            psyq::get_bit_fast(
                in_source.pack_, this_type::pack_TRANSITION_POSITION));
    }

    /// @brief 状態変化フラグを偽にする。
    public: void reset_transition()
    {
        this->pack_ = psyq::reset_bit_fast(
            this->pack_, this_type::pack_TRANSITION_POSITION);
    }
