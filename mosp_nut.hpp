#ifndef PSYQ_MOSP_NUT_HPP_
#define PSYQ_MOSP_NUT_HPP_
//#include "mosp_tree.hpp"

/// @cond
namespace psyq
{
    template<typename = PSYQ_MOSP_SPACE_DEFAULT> class mosp_nut;
}
/// @endcond
...
{
    /// thisが指す値の型。
    private: typedef mosp_nut this_type;

    //-------------------------------------------------------------------------
    /// mosp_nut で使うモートン空間の型。
    ...
    public: typedef concrete<psyq::geometric_sphere<typename this_type::space::coordinate::vector>>
        sphere;
    /// mosp_tree に取付可能な、線分の衝突判定オブジェクト。
    public: typedef concrete<psyq::geometric_segment<typename this_type::space::coordinate::vector>>
        segment;
    /// mosp_tree に取付可能な、半線分の衝突判定オブジェクト。
    public: typedef concrete<psyq::geometric_ray<typename this_type::space::coordinate::vector>>
        ray;
    /// mosp_tree に取付可能な、直方体の衝突判定オブジェクト。
    public: typedef concrete<psyq::geometric_cuboid<typename this_type::space::coordinate::vector>>
        cuboid;
    ...
    protected: mosp_nut():
        ...
        aabb_(
            typename this_type::space::coordinate::aabb(
                psyq::geometric_vector<typename this_type::space::coordinate::vector>::make(0),
                psyq::geometric_vector<typename this_type::space::coordinate::vector>::make(0)))
    {}

    /// *thisを mosp_tree から取り外す。
    public: virtual ~mosp_nut() {}
     ...
     */
    public: typename this_type::space::coordinate::aabb const& get_aabb() const
    {
        return this->aabb_;
    }
...
/** @brief  mosp_tree 空間分割木に取りつける、衝突判定オブジェクト。
    @tparam template_space @copydoc psyq::mosp_tree::space
    @tparam template_shape @copydoc psyq::mosp_nut_shape::shape
 */
template<typename template_space>
template<typename template_shape>
class psyq::mosp_nut<template_space>::concrete:
    public psyq::mosp_nut<template_space>
{
    /// thisが指す値の型。
    private: typedef concrete this_type;
    /// this_type の基底型。
    public: typedef psyq::mosp_nut<template_space> base_type;
    ...
    protected: void update_aabb() override
    {
        this->aabb_ = psyq::geometric_shape_aabb<
            typename base_type::space::coordinate::vector,
...
namespace psyq
{
    namespace test
    {
        inline void mosp_tree()
        {
            ...
            typedef psyq::mosp_nut<psyq_mosp_space> psyq_mosp_nut;
            ...
            psyq_mosp_nut::sphere local_mosp_sphere(
                psyq_mosp_nut::sphere::shape(
...
#endif // !defined(PSYQ_MOSP_NUT_HPP_)
